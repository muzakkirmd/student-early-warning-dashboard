...
NAMES = [
    'Aarav','Aditi','Aisha','Akash','Anil','Anjali','Arjun','Bhavna','Chirag','Deepak',
    ...
    with app.app_context():
        ...
        for i, name in enumerate(NAMES):
            ...
            student = Student(
                ...
                mentor_email=random.choice(MENTORS)
            )

            db.session.add(student)
            db.session.flush()

            db.session.add_all(generate_activity(student.id, profiles[i]))
