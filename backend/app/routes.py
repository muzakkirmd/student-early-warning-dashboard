...
def seed_database():
    ...
    for i, name in enumerate(NAMES):
        ...
        for j in range(30, 0, -1):
            ...
            db.session.add(ActivityLog(student_id=s.id, date=day, logged_in=logged_in,
...
@main.route('/api/reseed', methods=['GET', 'POST'])
def reseed_database():
    MentorActionLog.query.delete()
    ...
    for s in Student.query.all():
        ...
        if risk['level'] == 'high': high += 1
    db.session.commit()
    return jsonify({'message': 'Reseeded!', 'students': len(NAMES), 'high_risk': high})
...
def ingest_student():
    ...
    profiles = ['healthy', 'at_risk', 'dropped']
    ...
    for i in range(30, 0, -1):
        ...
        db.session.add(ActivityLog(
            student_id=student.id, date=day, logged_in=logged_in,
            session_minutes=session_min, assignment_submitted=submitted, quiz_score=quiz
        ))
    ...
    db.session.add(RiskScore(
        student_id=student.id, score=risk['score'],
        level=risk['level'], signals=risk['signals']
    ))
    db.session.commit()
...
def ingest_event():
    ...
    today = dt.today()
    existing_log = ActivityLog.query.filter_by(student_id=student.id, date=today).first()
    ...
    if existing_log:
        if event_type == 'login':
            ...
            if data.get('session_minutes'):
                existing_log.session_minutes = max(existing_log.session_minutes, int(data['session_minutes']))
    ...
    risk = calculate_risk_score(student.id)
    db.session.add(RiskScore(student_id=student.id, score=risk['score'], level=risk['level'], signals=risk['signals']))
    db.session.commit()

    return jsonify({'status': 'event_recorded', 'student': student.name,
